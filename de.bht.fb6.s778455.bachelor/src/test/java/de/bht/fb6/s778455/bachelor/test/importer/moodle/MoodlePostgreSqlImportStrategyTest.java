...
public class MoodlePostgreSqlImportStrategyTest extends NoLoggingTest {
    protected static final File IMPORT_FOLDER = new File(
            PATH_UNITTEST_DATA_FOLDER + "/importer/moodle/postgredump");
                ...
                if (1 == courseNum) {
                    ...
                    for (final Board board : course.getBoards()) {
                        if (0 == boardNum) {
                            ...
                            if (1 == boardNum) {
                                int threadNum = 0;
                                for (final BoardThread thread : board
                                        .getBoardThreads()) {
                                    if (0 == threadNum) {
                                        assertEquals("Optionale Aufgaben",
                                                thread.getTitle());
                                        assertEquals(board,
                                                thread.getBelongingBoard());

                                        // iterate through postings
                                        for (final Posting posting : thread
                                                .getPostings()) {
                                            // only one posting exists in
                                            // discussion = 1
                                            assertEquals("Optionale Aufgaben",
                                                    posting.getTitle());
                                            assertTrue(0 < posting.getContent()
                                                    .length());
                                        }
                                    } else {
                                        assertTrue(null != thread.getTitle());
                                        assertTrue(null != thread
                                                .getBelongingBoard());
                                    }
                                    threadNum++;
                                }
