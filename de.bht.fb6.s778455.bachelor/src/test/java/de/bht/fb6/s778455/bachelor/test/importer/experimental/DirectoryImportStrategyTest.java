...
public class DirectoryImportStrategyTest extends NoLoggingTest {
    ...
    public void testImportFromFile() throws GeneralLoggingException {
        final File testDir = IMPORT_FOLDER;
        ...
        for (final Course course : resultingSet) {
            ...
            for (final Board board : course.getBoards()) {
                ...
                final BoardThread boardThread : board.getBoardThreads()) {
                    if (0 == i) { // timestamp is smaller -> so it should be
                                  // first in the list
                                  // assertEquals( "Test thread",
                                  // boardThread.getTitle() );
                        // assertEquals( 1387364185,
                        // boardThread.getCreationDate()
                        // .getTime() );
                    } else if (1 == i) { // timestamp is greater -> so it
                                         // should be last in the list
                                         // assertEquals(
                                         // "1 Another test thread",
                        // boardThread.getTitle() );
                        // assertEquals( 1384093191,
                        // boardThread.getCreationDate()
                        // .getTime() );
                    }
                    i++;
                ...
                final List<Posting> postings = board.getBoardThreads().get(0)
                        .getPostings();
