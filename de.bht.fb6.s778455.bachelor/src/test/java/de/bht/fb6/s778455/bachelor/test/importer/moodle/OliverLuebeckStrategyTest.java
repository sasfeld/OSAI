...
public class OliverLuebeckStrategyTest extends NoLoggingTest {
    /**
     * Path to the folder containing the testdata to be imported. This is
     * hard-coded and not in a config file because we don't want to have unit
     * test configuration in production config files.
     */
    protected static final File IMPORT_FOLDER = new File(
            PATH_UNITTEST_DATA_FOLDER + "/importer/moodle/oliver_luebeck_test");
    ...
    public void testImportBoardFromFile() throws GeneralLoggingException {
        final LmsCourseSet courses = this.importStrategy
                .importBoardFromFile(IMPORT_FOLDER);
                ...
                for (final Board board : course.getBoards()) {
                    ...
                    if (0 == boardNum) {
                        ...
                        for (final BoardThread thread : board.getBoardThreads()) {
                            if (5 == threadNum) {
                                assertEquals("Mehr ueber generische Typen",
                                        thread.getTitle());
                                ...
                                int postingNum = 0;
                                for (final Posting posting : thread
                                        .getPostings()) {
                                    if (0 == postingNum) {
                                        assertEquals(1, posting.getId());
                                        assertEquals(null,
                                                posting.getParentPostingId());
                                        // we don't want CDATA annotations in
                                        // the content
                                        assertEquals(-1, posting.getContent()
                                                .indexOf("<![CDATA["));
                                        SimpleDateFormat format = new SimpleDateFormat(
                                                "dd/M/yyyy");
                                        assertEquals("12/11/2007",
                                                format.format(posting
                                                        .getCreationDate()));
