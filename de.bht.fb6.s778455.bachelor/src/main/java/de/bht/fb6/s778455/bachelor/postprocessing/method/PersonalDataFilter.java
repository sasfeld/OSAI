 ...
 *
 */
public class PersonalDataFilter extends AbstractPostprocessMethod {
    ...
    private void applyFilterOnPosting(Posting posting) 
    {
        if (null != posting.getContent()) {
            posting.setContent(StringUtil.removeMailAddresses(posting.getContent(), IReplacementTags.PERSONAL_DATA_REPLACEMENT_TAG));
        }
    }
